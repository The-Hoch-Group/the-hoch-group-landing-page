...
if (fs.existsSync(envPath)) {
  ...
  console.warn('⚠  No .env file found — falling back to environment variables.\n')
}
...
;(async () => {
  ...
  try {
    ...
    console.error('  • Wrong app password (re-generate in Zoho → Security → App Passwords)')
    console.error('  • Two-factor auth not enabled on the Zoho account (required for app passwords)')
    console.error('  • Firewall blocking port 465 — try port 587 with secure:false\n')
    process.exit(1)
  }
